...
class OmsorgspengerRefusjonPdfRequestMapperTest {
    ...
    void skal_opprette_pdfData() {
        ...
        var pdfData = OmsorgspengerRefusjonPdfRequestMapper.map(inntektsmeldingEntitet, ARBEIDSGIVER_NAVN, personInfo, ARBEIDSGIVER_IDENT);
        ...
        assertThat(pdfData.årForRefusjon()).isEqualTo(BigDecimal.valueOf(START_DATO.getYear()));
        ...
        assertThat(pdfData.omsorgspenger()).isNotNull();
        assertThat(pdfData.omsorgspenger().harUtbetaltPliktigeDager()).isTrue();
        assertThat(pdfData.omsorgspenger().fraværsPerioder()).hasSize(1);
        assertThat(pdfData.omsorgspenger().fraværsPerioder().getFirst().fom()).isEqualTo(FormatUtils.formaterDatoForLister(fraværsPeriode.getPeriode().getFom()));
        assertThat(pdfData.omsorgspenger().fraværsPerioder().getFirst().tom()).isEqualTo(FormatUtils.formaterDatoForLister(fraværsPeriode.getPeriode().getTom()));
        assertThat(pdfData.omsorgspenger().delvisFraværsPerioder()).hasSize(1);
        assertThat(pdfData.omsorgspenger().delvisFraværsPerioder().getFirst().dato()).isEqualTo(FormatUtils.formaterDatoForLister(delvisFraværsDag));
        assertThat(pdfData.omsorgspenger().delvisFraværsPerioder().getFirst().timer()).isEqualTo(delvisFraværstimer);
        assertThat(pdfData.omsorgspenger().trukketFraværsPerioder()).hasSize(1);
        assertThat(pdfData.omsorgspenger().trukketFraværsPerioder().getFirst().dato()).isEqualTo(FormatUtils.formaterDatoForLister(trukketFraværsDag));
    }
    ...
    void skal_opprette_med_endringsårsaker() {
        ...
        assertThat(pdfData.omsorgspenger()).isNotNull();
        assertThat(pdfData.omsorgspenger().harUtbetaltPliktigeDager()).isTrue();
        assertThat(pdfData.omsorgspenger().fraværsPerioder()).hasSize(2);
        assertThat(pdfData.omsorgspenger().fraværsPerioder().get(0).fom()).isEqualTo(FormatUtils.formaterDatoForLister(fraværsPeriode1.getPeriode().getFom()));
        assertThat(pdfData.omsorgspenger().fraværsPerioder().get(0).tom()).isEqualTo(FormatUtils.formaterDatoForLister(fraværsPeriode1.getPeriode().getTom()));
        assertThat(pdfData.omsorgspenger().fraværsPerioder().get(1).fom()).isEqualTo(FormatUtils.formaterDatoForLister(fraværsPeriode2.getPeriode().getFom()));
        assertThat(pdfData.omsorgspenger().fraværsPerioder().get(1).tom()).isEqualTo(FormatUtils.formaterDatoForLister(fraværsPeriode2.getPeriode().getTom()));
        assertThat(pdfData.omsorgspenger().delvisFraværsPerioder()).hasSize(0);
        assertThat(pdfData.omsorgspenger().trukketFraværsPerioder()).hasSize(0);
